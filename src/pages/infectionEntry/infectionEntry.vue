<template>
  <div id="container">
      ...
      <van-form  label-align="center"  v-for="(item,index) in infectionEntry" :key="index">
        <h3>{{item.disease_info_title}}</h3>
        <h4>{{item.children[0].disease_info_title}}</h4>
        <van-cell-group
         v-for="(itemLife) in item.children[0].children" :key="itemLife.id">
            <van-field
        ...
        <van-cell-group 
        v-for="(itemLife) in item.children[6].children" :key="itemLife.id"
        >
            <van-field
                v-model="itemLife.disease_info_title_value"
                :label="itemLife.disease_info_title"
        ...
        <h4>{{item.children[3].disease_info_title}}</h4>
            <van-field 
            name="checkboxGroup" 
            v-for="(item) in item.children[3].children" :key="item.id"
            :label="item.disease_info_title">
                <template #input>
                    ...
                    <van-checkbox 
                        :name="details.id" 
                        shape="square"
                        v-for="(details) in item.children" :key="details.id"
            ...
            <template #input>
                <van-checkbox-group v-model="antiInfectiveBox" style="width:100%">
                    <van-cell-group>
                        ...
                        v-for="(itemSigns,indexSigns) in item.children[2].children" :key="indexSigns"
                        clickable
                         @click="ChangeantiInfective(indexSigns)" 
                        :class="{addStyle:antiInfectiveBox.indexOf(itemSigns.id) != -1 }">
                            <template #right-icon>
                                <van-checkbox :name="itemSigns.id" ref="InfectiveCheckbox"/>
                            </template>
                        </van-cell>
                    </van-cell-group>
                </van-checkbox-group>
        ...
        <div style="margin: 16px;">
            <van-button round style="width:46%;margin-right:8%"  type="info" native-type="submit" class="reSubmit" size="normal">
            重置
            </van-button>
