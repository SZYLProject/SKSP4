...
export default {
  ...
  methods: {
    ...
    onClickRight(el) {
      if (el === 0) {
        if (this.$route.path != "/shock/table")
    ...
    //去往重症感染
    goInfection() {
      // if (this.$route.path != "/shock/InfectionEntry")
      // this.$router.push({path:'/shock/InfectionEntry',query:{type:'infection'}})
      // this.$router.push("/shock/table");
      this.title = "重症感染辅助决策系统";

      sessionStorage.setItem("systemTitle", "1");
      this.show = false;
      ...
      // this.$router.push({ path: "/shock/table" });
    },
    //去往重症休克
    goShock() {
      ...
      //   if (this.$route.path != "/shock/entry")
      // window.location.reload();
      // this.$router.push("/shock/table");

      // this.$router.push({ path: "/shock/entry", query: { type: "shock" } });
    },
    //去往首页
    goIndex() {
