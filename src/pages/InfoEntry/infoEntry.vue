  ...
  <div id="container">
    ...
    <div v-if="infoEntry">
      ...
      >
        ...
        <van-cell-group
          v-for="(itemLife, indexLife) in item.children[0].children"
          :key="indexLife"
        >
          <van-field
        ...
        <van-cell-group
          v-for="itemCheck in item.children[3].children"
          :key="itemCheck.id"
        >
          <van-field
          ...
          >
            <van-text slot="button" size="small" type="text">{{
              itemCheck.disease_info_title_unit
            }}</van-text>
          </van-field>
        </van-cell-group>
        ...
        <van-cell-group>
          <van-field
            ...
            v-for="itemNoCheck in item.children[4].children"
            :key="itemNoCheck.id"
          >
            <van-text slot="button" size="small" type="text">{{
              itemNoCheck.disease_info_title_unit
            }}</van-text>
          </van-field>
        </van-cell-group>
            ...
            >
              <van-cell-group>
                <van-cell
                  ...
                  :class="{
                    addStyle: SuperbornBox.indexOf(itemCheck.id) != -1
                  }"
                >
                  <template #right-icon>
...
export default {
  ...
  methods: {
    ...
    // async diseaseInfoSelect() {
    //   const id = sessionStorage.getItem("systemTitle");
    //   const data = {
    //     // num_hospital: this.$router.query.value.num_hospital,
    //     // patient_id: this.$router.query.value.patient_id
    //   };
    //   if (id == "1") {
    //     const result = await infectionPaient(data);
    //   }else if (id == '2'){
    //       const result = await diseaseInfoSelectSK(data)
    //   }
    // }
    getPaientInfoActions() {
  ...
  created() {
    this.getPaientInfoActions();
  },
  watch: {
    infoEntry(val) {
      console.log(val, "infoEntry");
    },
    infoEntry: {
      handler(value) {
        this.$nextTick(() => {
